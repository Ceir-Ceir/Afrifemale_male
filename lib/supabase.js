...
function sanitizeFileName(name) {
  if (!name) return `photo_${Date.now()}.jpg`;
  const lastDot = name.lastIndexOf('.');
  const ext = lastDot !== -1 ? name.substring(lastDot + 1).toLowerCase() : 'jpg';
  const base = lastDot !== -1 ? name.substring(0, lastDot) : name;
  
  const cleanExt = (ext === 'heic' || ext === 'heif') ? 'jpg' : ext;
  const cleanBase = base
    .replace(/[^a-zA-Z0-9_-]/g, '_')
    .replace(/_+/g, '_')
    .replace(/^_+|_+$/g, '');
    
  return `${cleanBase || 'photo'}_${Date.now()}.${cleanExt}`;
}
...
export async function uploadFile(folderPath, file) {
  ...
  if (!file || !(file instanceof Blob || file instanceof File)) {
    return { success: false, error: 'Invalid file payload provided.' };
  }
